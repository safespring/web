...
### Beskrivning
Visar den gemensamma videospelaren med stöd för HLS-strömmar, MP4 och undertexter. Shortcoden `video` ersätter den tidigare `streamed-video`.

### Kod
Implementationen finns i `layouts/shortcodes/video.html`, som använder `layouts/partials/components/video-player.html` och `static/js/video-player.js`.

### Parametrar

- `src` eller första positionsparametern: videons URL.
- `poster` eller andra positionsparametern: valfri förhandsbild.
- `subtitles`: valfri JSON-lista med undertexter.
...
Samma exempel med namngivna parametrar:
