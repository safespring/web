...
Betrakta följande Python-kod:```Python
...
Och `pulumi-config.yaml`-filen ser ut så här:```yaml
---
- name: pulumi-snipp
  flavor: l2.c2r4.100
  image: ubuntu-22.04
...
Nu kan vi tillämpa det önskade tillståndet genom att:```shell
...
```
Låt oss granska vad som skapades med hjälp av OpenStack CLI:```shell
...
instances:
  ...
  - name: pulumi-snapp
    ...
    security_groups:
      - ssh-from-the-world
```
Och sedan det uppdaterade Pulumi-programmet som kommer att implementera den logiska strukturen i YAML-filen:```python
"""An OpenStack Python Pulumi program"""
...
for i in config_dict['instances']:
  instances[i['name']] = compute.Instance(i['name'],
	...
	image_name = i["image"])
```
Låt oss köra Pulumi-programmet och se hur det önskade tillståndet för vår IaaS ändras enligt YAML-konfigurationsfilens struktur:```shell
(oscli) ubuntu@demo-jumphost:~/pulumi$ pulumi up
Previewing update (dev)
...
av tillståndet. Så, låt oss köra Pulumi-programmet igen och se vad som händer:```shell
(oscli) ubuntu@demo-jumphost:~/pulumi$ pulumi up
Previewing update (dev)
...
Låt oss kontrollera för att bekräfta.```shell
