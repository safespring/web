...
S3-bucket och därmed kör en fristående installation av Pulumi.```shell
...
Efter att ha konfigurerat Pulumi med OpenStack-mallen ser vi en fil som heter `__main__.py` i Pulumis arbetskatalog. Detta är en exempelfil som skapats av mallningsprocessen.```python
...
+--------------------------------------+------------------+----------------------------------------------------------------------------+
```
Så vi väljer att ändra filen `__main__.py` så här:```python
"""An OpenStack Python Pulumi program"""
...
därefter tillämpa den grafen mot OpenStack-API:et med Pulumi.```shell
(oscli) ubuntu@demo-jumphost:~/pulumi$
...
Om vi bryr oss om namnet på instansen kan vi helt enkelt lägga till det i Pulumi-programmet så här:```python
"""An OpenStack Python Pulumi program"""
...
Låt oss lägga till den här koden i Pulumi Python-programmet:```python
"""An OpenStack Python Pulumi program"""
