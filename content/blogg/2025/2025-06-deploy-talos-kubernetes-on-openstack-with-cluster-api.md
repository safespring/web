...
*Note: As the `ClusterResourceSet` get applied with labels, thus we recommend for the to be namespaced otherwise there is a risk of upgrading all clusters when updating them.*
...
*Note: We will make use of labels to demonstrate how the `ClusterResourceSet` get applied, to **all the clusters** in that namespace that have those labels.*
