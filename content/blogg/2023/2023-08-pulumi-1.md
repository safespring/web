...
date: "2023-08-17"
intro: "Combining imperative and declarative cloud automation with Python and Pulumi"
draft: false
tags: ["English"]
showthedate: true
card: ""
...
tools, their properties and their fitness for the purpose can be daunting. On
top of that comes the changing landscape in licensing and subscription,
...
* Basic understanding of Python programming language and the concepts of
  ...
  may help straitening out some concepts.
...
* A Pulumi supported operating system with API access to one of Safespring's
  ...
  - The APIs is firewalled so either you need to operate from an instance
    ([jumphost][jumpblog]) which is already in a Safespring Datacenter (then
    firewall openings is already in place), Or you must send an email to
    ...
    you access the API through NAT. Also, please tell us if the address changes
    so we can open the new and close the old.


## Setting up Pulumi

We recommend the [manual installation][pulmanual] for Linux because we advice against piping
content from the Internet directly to a shell. Of course the manual
installation is pretty easy to automate according to your own security polices
too.

When having a working Pulumi executable on you local machine (in this case a
...
blog post we will see how to change stack storage to a Safespring s3
...
parameters in order to maintain resources. The easiest way of quickly getting
...
If care about the name of the instance, we can just add it to the Pulumi program like this:

```
(oscli) ubuntu@demo-jumphost:~/pulumi$ cat __main__.py
"""An OpenStack Python Pulumi program"""
...
We have seen that we can use the Python (or other imperative programming
...
and imperative approach. In some sense the Pulumi program behaves like a
sequential recipe of tasks even though it eventually builds a similar resource
graph (state) as Terraform. This means that if change the order of events in
...
instance sill be member of is not yet defined in the program sequence. This is
different in Terraform where the ordering code statements is completely
irrelevant.

Next blog post we will be scaling things up and separating the program
