...
In the [previous blog post][pulblog1] we went through basic setup of Pulumi with
...
parameters in the code we read them from a dictionary which again is coming from
de-serialising data from the `pulumi-config.yaml` file. In addition, we create
...
```shell
V(oscli) ubuntu@demo-jumphost:~/pulumi$ pulumi up
...
So, we'll make changes to the Pulumi program so that it will accept
configuration of security groups and rules from the configuration yaml file and
add the list of security group memberships as parameter to the instances.

The new Python code also reflect a different structure in the yaml
...
`security_groups` sub-tree with rules for each security group as leafs under
each security group. Like this:
...
While applying state, we see that one of the planned actions failed due to API
timeout on the Openstack API. This sometimes happen and when it does it is nice
to have a tool that keeps track of current state and what was done even if some
actions failed. In this sense Pulumi behaves the same as Terraform and will
pick up the remaining changes on the next state application. So lets run the
...
the desired state should be equal to actual state.
...
Seems like Pulumi kept it's promises again. Note that we immediately can reach
