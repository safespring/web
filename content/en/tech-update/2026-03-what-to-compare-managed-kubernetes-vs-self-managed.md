...
Based on the current Safespring documentation, Safespring Kubernetes Engine is documented with the following platform characteristics:
...
Safespring does not present the service as if every operational concern is bundled into it. The current documentation recommends deploying your own observability stack, and cluster logs are not stored or monitored by default for Kubernetes Engine clusters.
