...
Se på følgende Python-kode:```Python
...
Og filen `pulumi-config.yaml` ser slik ut:```yaml
---
- name: pulumi-snipp
  flavor: l2.c2r4.100
  image: ubuntu-22.04
...
Nå kan vi anvende ønsket tilstand ved å:```shell
...
```
La oss undersøke hva som ble opprettet ved hjelp av OpenStack CLI:```shell
...
instances:
  ...
  - name: pulumi-snapp
    ...
    security_groups:
      - ssh-from-the-world
```
Og deretter det oppdaterte Pulumi-programmet som vil implementere den logiske strukturen i YAML-filen:```python
"""An OpenStack Python Pulumi program"""
...
i henhold til YAML-konfigurasjonsfilens struktur:```shell
(oscli) ubuntu@demo-jumphost:~/pulumi$ pulumi up
Previewing update (dev)
...
(oscli) ubuntu@demo-jumphost:~/pulumi$
```
Mens vi anvender tilstanden, ser vi at én av de planlagte handlingene feilet på grunn av en API-timeout mot OpenStack-API-et. Dette skjer av og til, og når det gjør det, er det fint å ha et verktøy som holder styr på gjeldende tilstand og hva som ble gjort, selv om noen handlinger feilet. I så måte oppfører Pulumi seg likt som Terraform og vil plukke opp de resterende endringene ved neste anvendelse av tilstanden. Så, la oss kjøre Pulumi-programmet på nytt og se hva som skjer:```shell
(oscli) ubuntu@demo-jumphost:~/pulumi$ pulumi up
Previewing update (dev)
...
La oss sjekke for å bekrefte det.```shell
