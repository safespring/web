...
The desktop megamenu service-card layout is also self-contained in `critical-layout.css`. It must not depend on `service-cards.css`, which is only selected on pages that render service-card grids.
...
Shared utility and cross-layout rules such as `.bg-white` and `.card-heading` remain in `main.css`; they must not depend on conditionally loaded card modules.
...
| `document-content.css` | `downloadpdf`, compliance document table, or footnote/document markup is present | Download metadata is generated by the default layout outside `.Content`; references and footnotes remain content-detected. |
...
| `horizontal-card.css` | Rendered content contains `safespring-horisontal-card-container` | Horizontal/custom-card shortcodes and list layout with symmetric content padding. |
...
| `service-cards.css` | Home, section list, or service-card markup | Service grids rendered by `li-index` and service shortcodes, including `20px` SVG icons inside stable `40px` circles. |
| `tooltips.css` | Rendered content contains tooltip markup | Tooltip trigger and visible text state. |
| `video-player.css` | Home, `videoURL`, or `data-video-player` markup | Video player, episode cards, subtitles, controls, and playlists, including SVG play-icon colors and chapter-list control geometry. |
